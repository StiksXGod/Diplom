"""Train and evaluate a PPO trading agent from the command line."""
...
def infer_feature_columns(data: pd.DataFrame) -> list[str]:
    ...
    Preferred RL features are the engineered indicators from
    ``src.data.features.DEFAULT_FEATURE_COLUMNS``. If they are unavailable, the
    function falls back to numeric columns except for identifiers and targets.
    """
...
def make_ppo_env(
    ...
    """Wrap TradingEnv for Stable-Baselines3 PPO training."""
...
def train_ppo(
    ...
    """Train a PPO agent on the training subset."""
...
def evaluate_ppo_agent(
    ...
    """Run the trained agent on the test environment and collect results."""
...
def save_evaluation_results(results: pd.DataFrame, output_path: Path) -> None:
    """Save test evaluation records to CSV."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Train a PPO trading agent on prepared feature data."
    )
    parser.add_argument(
        "--data_path",
        required=True,
...
def main() -> None:
    """Train PPO on a prepared dataset and save evaluation artifacts."""
    ...
    model_path = OUTPUT_DIR / f"ppo_{ticker.lower()}.zip"
    model.save(str(model_path))

    evaluation_results = evaluate_ppo_agent(
        model=model,
        test_df=test_df,
        feature_columns=feature_columns,
        initial_cash=args.initial_cash,
    ...
    results_path = OUTPUT_DIR / f"ppo_{ticker.lower()}_test_results.csv"
    ...
    metrics_csv_path = OUTPUT_DIR / f"ppo_{ticker.lower()}_metrics.csv"
    metrics_json_path = OUTPUT_DIR / f"ppo_{ticker.lower()}_metrics.json"
